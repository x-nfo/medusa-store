...
const getCartFields = (req: MedusaRequest) => {
  const queryFields = req.queryConfig?.fields ?? []
  if (!queryFields.length) {
    return defaultStoreCartFields
  }
  return Array.from(new Set([...queryFields, "shipping_address.metadata"]))
}

export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  const fields = getCartFields(req)
  const cart = await refetchCart(req.params.id, req.scope, fields)
  res.json({ cart })
}
